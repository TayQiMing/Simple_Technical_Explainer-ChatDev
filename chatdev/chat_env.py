...
class Phase(ABC):
    ...
    def self_reflection(self,
        ...
        elif phase_name == "StructureGeneration":
            question = """According to the structure content outline listed above, select three most important content sections title and its subsection title, then return the structure outline in the same format as listed above." """
        else:
...
class StructureGeneration(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['ideas'] = self.seminar_conclusion.split("<INFO>")[-1].lower().replace("\n", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['ideas'] = self.seminar_conclusion
        else:
            chat_env.env_dict['ideas'] = "I have no idea on the structure online."
        log_and_print_online("**[Content Structure]**:\n\n {}".format(get_info(chat_env.env_dict['directory'], self.log_filepath)))
        return chat_env
...
class ContentGeneration(Phase):
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        filename = self.seminar_conclusion.split("<FILENAME>")[1].split("</FILENAME>")[0]
        chat_env._update_manuals(self.seminar_conclusion,filename)
        chat_env.rewrite_manuals()
        log_and_print_online("**[Generated Content]**:\n\n {}".format(get_info(chat_env.env_dict['directory'], self.log_filepath)))
        return chat_env
